...
template <class T> int LinkedList<T>::findData(T value) {
  ...
  while (current != nullptr) {
    if (current->data == value) {
      return position; // Retorna la posición si encuentra el valor
    }
...
//complejidad O(n)
template <class T>
LinkedList<T>::LinkedList(const LinkedList<T> &other) : head(nullptr), tail(nullptr), numElements(0) {
    NodeLinkedList<T> *current = other.head;
    while (current != nullptr) {
        addLast(current->data); // Añade una copia de cada elemento al final de la nueva lista
        current = current->next;
    }
}
